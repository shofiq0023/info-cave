...
import com.infoCave.dao.CreateBlogDAO;
...
public class CreateBlog extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String author = (String) session.getAttribute("userAuthorName");
		String snippets = request.getParameter("blogSnippet");
		String detail = request.getParameter("blogDetail");
		int userId = (int) session.getAttribute("userId");
		
		CreateBlogDAO dao = new CreateBlogDAO();
		
		try {
			if(dao.insertBlog(title, author, snippets, detail, userId)) {
